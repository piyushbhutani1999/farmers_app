...
class AddProductInfoForm(forms.ModelForm):
    ...
    title = forms.CharField(widget=forms.TextInput(attrs={
                            'placeholder': 'Title', 'class': 'border w-100 p-2 bg-white text-capitalize'}))
    description = forms.CharField(widget=forms.Textarea(
        attrs={'placeholder': 'Describe Your Product', 'class': 'border p-3 w-100'}))
    price = forms.IntegerField(widget=forms.NumberInput(
        attrs={'placeholder': 'Price', 'class': 'border-0 py-2 w-100 price'}))
    image = forms.ImageField(widget=forms.FileInput(
        attrs={'class': 'form-control-file d-none'}))
